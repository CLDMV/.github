...
**Prereqs** — the bot App must have Organization → Members: read (to detect org members for exemption) and Contents: write on the `CLDMV/.cla-signatures` ledger repo (to write signature files). The ledger repo itself must exist and be seeded from [`examples/repo-seeds/.cla-signatures/`](../repo-seeds/.cla-signatures/). The binding CLA text lives in the private ledger at `cla-versions/v<X.Y>.md`; a public mirror is kept in the seed for reference. This `.github` repo deliberately has no root-level `CLA.md` — if it did, the bot would pick it up as a fallback CLA source.
