...
const sign = (process.env.INPUT_SIGN || 'auto').toLowerCase();
const annotate = (process.env.INPUT_ANNOTATE || 'auto').toLowerCase();
...
const push = String(process.env.INPUT_PUSH || 'true').toLowerCase() !== 'false';
	...
	const res = await run({ token, repo, raw, runCheck, runCreate, runUpdate, sign, annotate, tagger_name, tagger_email, gpg_private_key, gpg_passphrase, push });
