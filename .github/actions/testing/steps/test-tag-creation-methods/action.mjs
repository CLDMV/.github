...
function analyzeToken(token) {
	const tokenType = token.startsWith('ghs_') ? 'app_token' : 
	                 token.startsWith('ghp_') ? 'personal_token' : 
	                 token.startsWith('github_pat_') ? 'fine_grained_token' :
 ...
 */
async function createTagViaGit({ 
	tag, 
	targetCommit, 
	enableSign 
}) {
	...
	try {
		...
		const isVerified = showResult.includes('gpg: Good signature') || 
		                  showResult.includes('Signature made');
