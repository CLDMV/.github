...
async function main() {
  try {
    ...
    if (foundNonBot) {
      ...
      const fixedTagsList = nonBotTags.map(tag => 
        `${tag.tag} (was: ${tag.currentSigner} <${tag.currentEmail}>)`
      );
      ...
      // Set outputs
      core.setOutput('non-bot-tags-found', 'true');
      core.setOutput('fixed-tags', fixedTagsList.join('\n'));
      core.setOutput('non-bot-tags-json', JSON.stringify(jsonPayload));
      ...
      core.setOutput('non-bot-tags-found', 'false');
      core.setOutput('fixed-tags', '');
      core.setOutput('non-bot-tags-json', '[]');
    }
